...
export default function About() {
  ...
  // Simulating initial data fetch or page initialization
  useEffect(() => {
    const timer = setTimeout(() => {
      setLoading(false);
    }, 800); // Quick professional delay
    return () => clearTimeout(timer);
  }, []);

  // 🔥 Refined Animated Stats - Only runs after loading is false
  useEffect(() => {
    if (loading) return;
    ...
    <div className="min-h-screen bg-[#F9F9F7] text-[#1a1a1a] font-sans selection:bg-yellow-200">
      ...
      <div className="sticky top-0 z-30 bg-white/80 backdrop-blur-xl border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between gap-4">
          ...
          <div className="hidden md:block text-[10px] font-bold text-gray-400 uppercase tracking-widest">
            {loading ? "Initializing..." : "Institutional Publication • AMA Lipa"}
          </div>
        </div>
      </div>
      ...
      <main className="max-w-7xl mx-auto p-6 py-12 space-y-16">
        ...
        {loading ? (
          ...
          <>
            {/* 📊 STATS GRID */}
            <section className="grid md:grid-cols-3 gap-6 animate-in fade-in duration-700">
            ...
            {/* 🎯 MISSION & VISION */}
            <section className="grid md:grid-cols-2 gap-8 animate-in slide-in-from-bottom-4 duration-700">
            ...
            {/* 🕰 TIMELINE */}
            <section className="bg-white p-10 md:p-14 rounded-[2.5rem] border border-gray-100 shadow-sm animate-in slide-in-from-bottom-4 duration-1000">
