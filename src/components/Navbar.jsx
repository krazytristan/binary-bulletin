...
export default function Navbar() {
  ...
  return (
    <nav className="sticky top-0 z-50 bg-blue-900 text-white font-sans border-b-4 border-blue-950 shadow-md">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex justify-between items-center gap-4">
        
        {/* LOGO */}
        <Link to="/" className="flex items-center gap-3 shrink-0">
          <img src="/binary-logo.png" alt="Logo" className="h-8 w-8 object-contain bg-white rounded-full p-0.5" />
          <h1 className="font-serif font-black text-xl hidden lg:block uppercase tracking-tight text-white drop-shadow-sm">The Binary Bulletin</h1>
        </Link>
        ...
        <div className="hidden lg:flex items-center gap-6 text-[10px] font-bold uppercase tracking-widest">
          {navLinks.map((link) => (
            <Link key={link.path} to={link.path} className="relative group py-2">
              <span className={`transition-colors ${location.pathname === link.path ? "text-amber-400" : "text-blue-100 hover:text-white"}`}>
                {link.name}
              </span>
              <span className={`absolute left-0 bottom-0 h-[2px] bg-amber-400 transition-all ${location.pathname === link.path ? "w-full" : "w-0 group-hover:w-full"}`}></span>
            </Link>
          ))}
        </div>
        ...
        <div className="relative flex-1 max-w-[180px] xs:max-w-[220px] sm:max-w-xs ml-auto lg:ml-0" ref={searchRef}>
          <div className="relative group">
            <input
              ...
              className="w-full bg-blue-950 border border-blue-800 rounded-none py-2 pl-4 pr-10 text-[10px] font-bold uppercase tracking-widest text-white placeholder:text-blue-300 outline-none focus:border-amber-400 focus:bg-blue-900 transition-all"
            />
            <div className="absolute right-3 top-1/2 -translate-y-1/2 text-blue-300 group-focus-within:text-amber-400">
          ...
          {showResults && searchQuery.length > 1 && (
            <div className="absolute top-full mt-2 left-0 right-0 bg-white border-2 border-blue-900 shadow-xl overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
              <div className="max-h-[350px] overflow-y-auto p-2 scrollbar-thin scrollbar-thumb-blue-900/10">
                {results.length > 0 ? (
                  results.map((res, index) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-serif font-bold text-gray-900 truncate">{res.title}</p>
                        <p className="text-[9px] font-sans text-red-800 uppercase tracking-widest font-bold">{res.category}</p>
                      </div>
                    </button>
                  ))
                ) : (
